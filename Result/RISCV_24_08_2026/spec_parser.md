...
**Cấu trúc tài liệu:** §1 Target → §2 Parameter → §3 Overview (TOP) → §4 Interconnect → §5+ Sub-module.
...
|---|---|---|---|
| **F01** | RV32I base integer instruction set (40 lệnh) | ✅ | — |
...
| U19 | `rv32im_clint` | seq | SoC (ngoài lõi) | §19 |
...
| Memory latency | Hệ quả |
|---|---|
| 0 cycle (combinational read) | IF không stall, đạt IPC lý thuyết |
| ≥1 cycle | IF stall tương ứng — hành vi đúng nhưng chậm |
...
| CH12a | `mem_stage` | `csr_file` | `w_csr_en`, `w_csr_op`, `w_csr_addr`, `w_csr_wdata` | 1+2+12+XLEN | N | CSR access request |
| CH12b | `csr_file` | `mem_stage` | `w_csr_rdata`, `w_csr_illegal` | XLEN+1 | N | CSR read data + illegal flag |
| CH13a | `mem_stage` | `trap_ctrl` | `w_exc_valid`, `w_exc_code`, `w_exc_tval`, `w_mem_pc`, `w_mem_instr_valid`, `w_mem_outstanding`, `w_sys_mret`, `w_sys_fencei` | — | N | Nguồn trap từ tầng commit |
...
| | `exc_tval` | `PR_XLEN` |
| Trace | `instr` | `PR_INSTR_W` (chỉ khi `PR_TRACE_EN`) |
...
|---|---|---|
| `ST_IDLE` | `!i_stall` | Phát `o_imem_req_valid = 1` |
| `ST_WAIT` | Đã bắt tay, chờ `i_imem_rsp_valid` | `o_if_busy = 1` |
| — | `i_imem_rsp_valid` | Ghi `instr` vào IF/ID, quay lại `ST_IDLE` |
...
| `PR_RF_IMPL` | `int unsigned` | `0` | `0`=flop array · `1`=RAM-style (`PR_RF_RESET_EN` bị bỏ qua) |
...
> Với `PR_RF_IMPL = 1`, bypass phải làm bằng logic ngoài (RAM macro không có write-first).
...
```
Khi `o_ex_busy = 1`, `hazard_ctrl` stall IF/ID/EX và chèn bubble vào MEM.
`i_flush` phải huỷ MULDIV đang chạy (reset FSM về IDLE).
...
| Status | `o_done` | out | 1 | Kết quả sẵn sàng |
...
| | `o_csr_rdata` | out | `PR_XLEN` | Giá trị **trước** khi ghi |
| | `o_csr_illegal` | out | 1 | |
...
| Counter | `i_instr_retire` | in | 1 | Tăng `minstret` |
...
| `0x310` | `mstatush` | RO | `'0` | RV32-only, mọi field = 0 |
...
phải bị huỷ. `mem_stage` đảm bảo `o_csr_wr_en = 0` khi `i_trap_taken = 1`.
...
|---|---|---|
| 1 | `o_trap_taken` | Direct: `{i_mtvec[31:2], 2'b00}`<br>Vectored (`PR_MTVEC_VEC_EN=1`, `mtvec.MODE=1`, interrupt): `{i_mtvec[31:2],2'b00} + (code << 2)` |
| 2 | `i_sys_mret` | `i_mepc` |
| 3 | `i_sys_fencei` | `i_mem_pc_plus4` |

Cả 3 trường hợp đều flush IF, ID, EX.
...
Khi `PR_FWD_EN = 0`: luôn trả `FWD_NONE`, và điều kiện stall §18.5 phải mở rộng để
bao mọi RAW với EX/MEM và MEM/WB (không chỉ load-use).
...
| Sự kiện | Nguồn | Stall | Flush (bubble) |
|---|---|---|---|
| I-bus chưa response | `i_if_busy` | IF | — |
| Load-use | `w_load_use` | IF, ID | EX |
| MULDIV đang chạy | `i_ex_busy` | IF, ID, EX | MEM |
| D-bus chưa response | `i_mem_busy` | IF, ID, EX, MEM | WB |
| Branch/Jump taken | `i_redirect_ex_valid` | — | IF, ID |
| Trap taken | `i_trap_taken` | — | IF, ID, EX, **MEM** |
| MRET / FENCE.I | `i_redirect_mem_valid` | — | IF, ID, EX |
...
| # | Hạng mục | Ảnh hưởng |
|---|---|---|
| O1 | **Target công nghệ** — FPGA (board nào?) hay chỉ mô phỏng? | Quyết định default của `PR_MULT_IMPL` và `PR_RF_IMPL` |
| O2 | **Timing constraint** — chưa có mục tiêu tần số | Critical path dự kiến: `fwd mux → ALU → branch resolve → PC mux` |
