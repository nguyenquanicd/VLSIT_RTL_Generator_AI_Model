...
task automatic tc_010_latency_measurement();
  ...
  fork
    ...
    begin : receiver
      ...
      t_first_beat = $realtime;
      axis_recv_m_beat(beat1, beat1_last, beat1_keep);
    end
  join

  latency_cycles = (t_first_beat - t_accept) / LP_CLK_PERIOD_NS;
  $display("[INFO] TC-010 latency_measurement: %.1f cycles (accept -> first M_AXIS beat), soft target 2-4 cycles", latency_cycles);
